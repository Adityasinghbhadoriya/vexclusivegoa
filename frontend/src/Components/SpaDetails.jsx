...
const SpaDetails = () => {
  ...
  const actions = [
    ...
    },
    {
      icon: FaPhoneAlt,
      label: "Call",
      sub: "Book your session",
      onClick: () => (window.location.href = `tel:${spa.phone}`),
    },
    {
      icon: FaInstagram,
      label: "Instagram",
  ...
  return (
    <div className="min-h-screen bg-stone-50 pb-12">
      <style>{`
        ...
        @keyframes pulseGlow { 0%,100%{box-shadow:0 10px 30px -10px rgba(16,185,129,.5)}50%{box-shadow:0 15px 40px -10px rgba(16,185,129,.8)} }
      ...
      >
        ...
        <div className="absolute bottom-0 left-0 right-0 z-10 px-6 pb-8 animate-fade-in-up">
          <div className="inline-flex items-center gap-1.5 rounded-full bg-white/10 backdrop-blur-md border border-white/20 px-3 py-1.5 mb-4">
            <HiSparkles className="text-emerald-300 text-xs" />
          ...
          <p className="text-white/80 text-base font-light mb-3">{spa.category}</p>
          <div className="flex items-center gap-4 text-white/90 text-sm">
            <span className="flex items-center gap-1.5">
              <FaStar className="text-emerald-300" />
              <span className="font-semibold">{spa.rating}</span>
              <span className="text-white/60">({spa.reviews})</span>
            </span>
            <span className="h-3 w-px bg-white/30" />
            <span className="flex items-center gap-1.5">
              <FaMapMarkerAlt className="text-xs" />
              <span className="text-xs">{spa.location}</span>
            </span>
          </div>
        </div>
      ...
      <div className="relative -mt-8 mx-4 rounded-t-[2rem] bg-stone-50 px-6 pt-8 pb-6 shadow-2xl">
        <div className="relative overflow-hidden rounded-3xl p-[1.5px] shadow-[0_10px_40px_-10px_rgba(16,185,129,0.5)] animate-scale-in bg-gradient-to-br from-emerald-500 via-teal-500 to-cyan-500">
          <div className="relative rounded-[calc(1.5rem-1.5px)] bg-gradient-to-br from-emerald-500 via-teal-500 to-cyan-500 px-6 py-7 overflow-hidden">
        ...
        <div className="mt-5 flex items-center gap-3 rounded-2xl bg-stone-100 px-4 py-3.5 border border-stone-200">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm font-semibold text-stone-900">{spa.hours}</p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            {actions.map(({ icon: Icon, label, sub, onClick }, i) => (
              <button
                ...
                className="group relative overflow-hidden rounded-2xl bg-white border border-stone-200 p-4 text-left shadow-sm hover:shadow-lg hover:-translate-y-1 hover:border-emerald-300 transition animate-fade-in-up opacity-0"
              >
                <div className="grid place-items-center h-11 w-11 rounded-xl bg-gradient-to-br from-emerald-500 to-teal-500 text-white mb-3 group-hover:scale-110 group-hover:rotate-3 transition shadow-md">
